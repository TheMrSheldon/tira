...
export default {
  ...
  methods: {
    runEvaluation(isActive: any) {
      ...
      get(inject("REST base URL")+`/grpc/${this.run.vm_id}/run_eval/${this.run.dataset_id}/${this.run.run_id}`)
    ...
    deleteRun(isActive: any) {
      ...
      get(inject("REST base URL")+`/grpc/${this.run.vm_id}/run_delete/${this.run.dataset_id}/${this.run.run_id}`)
